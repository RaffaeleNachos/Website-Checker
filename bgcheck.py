...
while True:
	...
	#print(content) 
	if ((str(content).find("Under restocking") != -1) or (str(content).find("Expected restock on") != -1)): #if the keyword isn't found the code go to the else and send the email
		time.sleep(60) #or it waits 60 seconds and download the HTML code again
		continue
	
	else:
		msg = 'The product on Banggood is avaiable! Check the Website, now!'
